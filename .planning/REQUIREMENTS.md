...
- [x] **FOUND-01**: Django project is structured with separate apps (organizations, ingestion, normalization, audit, emissions)
- [x] **FOUND-02**: PostgreSQL database is configured with proper settings
- [x] **FOUND-03**: All models can be migrated cleanly from scratch
- [x] **FOUND-04**: Organization model exists as the multi-tenancy root (all records belong to an Organization)

### Ingestion — SAP Source

- [x] **SAP-01**: System accepts SAP fuel/procurement CSV upload via REST API endpoint
...
- [x] **SAP-06**: Invalid/malformed SAP rows do not abort the batch — they are stored with error detail

### Ingestion — Utility Electricity Source

- [x] **UTIL-01**: System accepts utility electricity CSV upload via REST API endpoint
...
- [x] **UTIL-05**: Invalid/malformed utility rows do not abort the batch

### Ingestion — Corporate Travel Source

- [x] **TRVL-01**: System exposes a REST API endpoint to trigger mocked corporate travel data ingestion
...
- [x] **TRVL-05**: Travel ingestion creates RawRecords with travel-specific metadata

### Normalization

- [x] **NORM-01**: All ingested records from all 3 sources are normalized into NormalizedEmissionRecord
...
- [x] **NORM-07**: The specific emission_factor_id used at calculation time is stored on the record (snapshot)

### Audit & Traceability

- [x] **AUDIT-01**: Every NormalizedEmissionRecord references its RawRecord, RawUpload, and source DataSource
...
- [x] **AUDIT-05**: NormalizedEmissionRecord has a status field supporting approval workflow states (pending_review → approved / rejected)

### Validation

- [x] **VALID-01**: Each source has a validation schema (pydantic) that checks required fields and types
...
- [x] **VALID-05**: A REST API endpoint returns all suspicious rows across an organization

### REST APIs

- [x] **API-01**: POST /api/ingestion/sap/upload/ — upload SAP CSV
...
- [x] **API-07**: GET /api/emissions/suspicious/ — list flagged suspicious rows

### Mock Data & Seed Data

- [x] **DATA-01**: Realistic SAP CSV mock dataset (50+ rows, messy columns, mixed dates, multiple plant codes)
...
- [x] **DATA-05**: Management command or fixture to load seed emission factors and sample organizations
